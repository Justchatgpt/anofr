--设置按键
RestorationSetBinding = CreateFrame("Frame")
RestorationSetBinding:RegisterEvent("PLAYER_ENTERING_WORLD")
RestorationSetBinding:RegisterEvent("ACTIVE_TALENT_GROUP_CHANGED")
--当选择其他职责时排队准备进入副本时,刚好玩家在战斗中,导致不能正确设置按键.因此改为OnUpdate模式

function RestorationSetBindingSaveBindings()
...
function RestorationSetBinding_OnEvent(self, event, ...)
	RestorationSetBinding_OnEventInterval = RestorationSetBinding_OnEventInterval or GetTime()
	if GetTime() - RestorationSetBinding_OnEventInterval > 1 then
		if DA_GetSpecialization() == 105 then
			DA_GetAssignSpellIDs(DA_CastLevelColorCache_Restoration)
			--获取当前最高等级技能法术ID
			--print("当前[回春术]法术ID: " .. Rejuvenation_SpellID)
		end
		
		if not UnitAffectingCombat("player") and DA_GetSpecialization() == 105 then
			if not GetBindingKey("WoWAssistant_Replace") then
				SetBinding("F9", "WoWAssistant_Replace")
				--切换
				RestorationSetBindingSaveBindings()
			end
		...
		RestorationSetBinding_OnEventInterval = nil
	end
end

function RestorationSetMacroButton()
	if DA_GetSpecialization() == 105 then
		local macros = {
			...
			{framename = "DA_CAST_LIFEBLOOM", button = "CTRL-INSERT", macrotext = "/cast 生命绽放"},--78
			{framename = "DA_CAST_REJUVENATION", button = "CTRL-DELETE", macrotext = "/cast 回春术"},--79
			{framename = "DA_CAST_REGROWTH", button = "CTRL-HOME", macrotext = "/cast 愈合"},--80
			{framename = "DA_CAST_CENARION_WARD", button = "CTRL-END", macrotext = "/cast 塞纳里奥结界"},--81
			{framename = "DA_CAST_NATURE_SWIFTNESS", button = "CTRL-PAGEUP", macrotext = "/cast 自然迅捷"},--82
			{framename = "DA_CAST_WILD_GROWTH", button = "CTRL-PAGEDOWN", macrotext = "/cast 野性成长"},--83
			{framename = "DA_CAST_BARKSKIN", button = "CTRL-UP", macrotext = "/cast 树皮术"},--84
			{framename = "DA_CAST_NOURISH", button = "CTRL-LEFT", macrotext = "/cast 滋养"},--85
			{framename = "DA_CAST_TRANQUILITY", button = "CTRL-DOWN", macrotext = "/cast 宁静"},--86
			{framename = "DA_CAST_INNERVATE", button = "CTRL-RIGHT", macrotext = "/cast 激活"},--87
			{framename = "DA_CAST_REBIRTH", button = "ALT-INSERT", macrotext = "/cast 复生"},--88
			{framename = "DA_CAST_SWIFTMEND", button = "ALT-DELETE", macrotext = "/cast 迅捷治愈"},--89
			{framename = "DA_CAST_NATURE_CURE", button = "ALT-HOME", macrotext = "/cast 自然之愈"},--90
			{framename = "DA_CAST_SOOTHE", button = "ALT-END", macrotext = "/cast 安抚"},--97
			{framename = "DA_CAST_INCARNATION_TREE_OF_LIFE", button = "ALT-PAGEUP", macrotext = "/cast !化身：生命之树"},--92
			{framename = "DA_CAST_CONVOKE_THE_SPIRITS", button = "ALT-PAGEDOWN", macrotext = "/cast 万灵之召"},--93
			{framename = "DA_CAST_EFFLORESCENCE", button = "ALT-UP", macrotext = "/cast [@player]百花齐放"},--94
			{framename = "DA_CAST_OVERGROWTH", button = "ALT-LEFT", macrotext = "/cast 过度生长"},--95
			{framename = "DA_CAST_IRONBARK", button = "ALT-DOWN", macrotext = "/cast 铁木树皮"},--96
			{framename = "DA_CAST_FLOURISH", button = "ALT-RIGHT", macrotext = "/cast 繁盛"},--97
			{framename = "DA_CAST_INVIGORATE", button = "SHIFT-INSERT", macrotext = "/cast 鼓舞"},--98
			{framename = "DA_CAST_RENEWAL", button = "SHIFT-DELETE", macrotext = "/cast 甘霖"},--99
			{framename = "DA_CAST_BERSERKING", button = "SHIFT-HOME", macrotext = "/cast 狂暴(种族特长)"},--100
			{framename = "DA_CAST_HEART_OF_THE_WILD", button = "SHIFT-END", macrotext = "/cast 野性之心"},--101
			{framename = "DA_CAST_NATURE_VIGIL", button = "SHIFT-PAGEUP", macrotext = "/cast 自然的守护"},--102
			{framename = "DA_CAST_RAKE", button = "SHIFT-PAGEDOWN", macrotext = "/cast 斜掠"},--103
			{framename = "DA_CAST_SHRED", button = "SHIFT-UP", macrotext = "/cast 撕碎"},--104
			{framename = "DA_CAST_BEAR_FORM", button = "SHIFT-LEFT", macrotext = "/cast [nostance:1]熊形态"},--105
			{framename = "DA_CAST_CAT_FORM", button = "SHIFT-DOWN", macrotext = "/cast [nostance:2]猎豹形态"},--106
			{framename = "DA_CAST_GROVE_GUARDIANS", button = "SHIFT-RIGHT", macrotext = "/cast 林莽卫士"},--107
			{framename = "DA_CAST_RIP", button = "ALT-CTRL-INSERT", macrotext = "/cast 割裂"},--108
			{framename = "DA_CAST_FEROCIOUS_BITE", button = "ALT-CTRL-DELETE", macrotext = "/cast 凶猛撕咬"},--109
			{framename = "DA_CAST_MAIM", button = "ALT-CTRL-HOME", macrotext = "/cast 割碎"},--110
			{framename = "DA_CAST_THRASH", button = "ALT-CTRL-END", macrotext = "/cast 痛击"},--111
			{framename = "DA_CAST_SWIPE", button = "ALT-CTRL-PAGEUP", macrotext = "/cast 横扫"},--112
			{framename = "DA_CAST_SKULL_BASH", button = "ALT-CTRL-PAGEDOWN", macrotext = "/cast 迎头痛击"},--113
			{framename = "DA_CAST_INCAPACITATING_ROAR", button = "ALT-CTRL-UP", macrotext = "/cast 夺魂咆哮"},--114
			{framename = "DA_CAST_BASH", button = "ALT-CTRL-LEFT", macrotext = "/cast 蛮力猛击"},--115
			{framename = "DA_CAST_MOONFIRE", button = "ALT-CTRL-DOWN", macrotext = "/cast 月火术"},--116
			{framename = "DA_CAST_SUNFIRE", button = "ALT-CTRL-RIGHT", macrotext = "/cast 阳炎术"},--117
			{framename = "DA_CAST_WRATH", button = "ALT-SHIFT-INSERT", macrotext = "/cast 愤怒"},--118
			{framename = "DA_CAST_STARFIRE", button = "ALT-SHIFT-DELETE", macrotext = "/cast 星火术"},--119
			{framename = "DA_CAST_STARSURGE", button = "ALT-SHIFT-HOME", macrotext = "/cast 星涌术"},--120
			...
			{framename = "DA_CAST_THORNS", button = "ALT-SHIFT-PAGEDOWN", macrotext = "/cast 荆棘术"},--123
...
RestorationSetBinding:SetScript("OnEvent", RestorationSetMacroButton)
RestorationSetBinding:SetScript("OnUpdate", RestorationSetBinding_OnEvent)