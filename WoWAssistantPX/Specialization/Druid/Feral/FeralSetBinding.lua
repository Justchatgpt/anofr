--设置按键
FeralSetBinding = CreateFrame("Frame")
FeralSetBinding:RegisterEvent("PLAYER_ENTERING_WORLD")
FeralSetBinding:RegisterEvent("ACTIVE_TALENT_GROUP_CHANGED")
--当选择其他职责时排队准备进入副本时,刚好玩家在战斗中,导致不能正确设置按键.因此改为OnUpdate模式

function FeralSetBindingSaveBindings()
...
function FeralSetBinding_OnEvent(self, event, ...)
	FeralSetBinding_OnEventInterval = FeralSetBinding_OnEventInterval or GetTime()
	if GetTime() - FeralSetBinding_OnEventInterval > 1 then
		if DA_GetSpecialization() == 103 then
			DA_GetAssignSpellIDs(DA_CastLevelColorCache_Feral)
			--获取当前最高等级技能法术ID
			--print("当前[回春术]法术ID: " .. Rejuvenation_SpellID)
		end
		
		if not UnitAffectingCombat("player") and DA_GetSpecialization() == 103 then
			if not GetBindingKey("WoWAssistant_Replace") then
				SetBinding("F9", "WoWAssistant_Replace")
				--切换
				FeralSetBindingSaveBindings()
			end
		...
		FeralSetBinding_OnEventInterval = nil
	end
end

function FeralSetMacroButton()
	if DA_GetSpecialization() == 103 then
		local macros = {
			...
			{framename = "DA_CAST_RAKE", button = "CTRL-INSERT", macrotext = "/cast 斜掠"},--78
			{framename = "DA_CAST_REJUVENATION", button = "CTRL-DELETE", macrotext = "/cast [@player]回春术"},--79
			{framename = "DA_CAST_THRASH", button = "CTRL-HOME", macrotext = "/cast 痛击"},--80
			{framename = "DA_CAST_REMOVE_CORRUPTION", button = "CTRL-END", macrotext = "/cast 清除腐蚀"},--81
			{framename = "DA_CAST_RIP", button = "CTRL-PAGEUP", macrotext = "/cast 割裂"},--82
			{framename = "DA_CAST_WILD_GROWTH", button = "CTRL-PAGEDOWN", macrotext = "/cast 野性成长"},--83
			{framename = "DA_CAST_MAIM", button = "CTRL-UP", macrotext = "/cast 割碎"},--84
			{framename = "DA_CAST_SKULL_BASH", button = "CTRL-LEFT", macrotext = "/cast 迎头痛击"},--85
			...
			{framename = "DA_CAST_SHRED", button = "ALT-PAGEUP", macrotext = "/cast 撕碎"},--92
			{framename = "DA_CAST_TIGER_FURY", button = "ALT-PAGEDOWN", macrotext = "/cast 猛虎之怒"},--93
			{framename = "DA_CAST_PRIMAL_WRATH", button = "ALT-UP", macrotext = "/cast 原始之怒"},--94
			{framename = "DA_CAST_SURVIVAL_INSTINCTS", button = "ALT-LEFT", macrotext = "/cast 生存本能"},--95
			{framename = "DA_CAST_BERSERK", button = "ALT-DOWN", macrotext = "/cast 狂暴"},--96
			{framename = "DA_CAST_SWIPE", button = "ALT-RIGHT", macrotext = "/cast 横扫"},--97
			{framename = "DA_CAST_ADAPTIVE_SWARM", button = "SHIFT-INSERT", macrotext = "/cast 激变蜂群"},--98
			...
			{framename = "DA_CAST_FERAL_FRENZY", button = "SHIFT-END", macrotext = "/cast 野性狂乱"},--101
			{framename = "DA_CAST_FEROCIOUS_BITE", button = "SHIFT-PAGEUP", macrotext = "/cast 凶猛撕咬"},--102
			{framename = "DA_CAST_BARKSKIN", button = "SHIFT-PAGEDOWN", macrotext = "/cast 树皮术"},--103
			{framename = "DA_CAST_FRENZIED_REGENERATION", button = "SHIFT-UP", macrotext = "/cast 狂暴回复"},--104
			...
			{framename = "DA_CAST_CAT_FORM", button = "SHIFT-RIGHT", macrotext = "/cast [nostance:2]猎豹形态"},--107
			...
			{framename = "DA_CAST_MOONFIRE", button = "ALT-CTRL-HOME", macrotext = "/cast 月火术"},--110
			{framename = "DA_CAST_MAIM", button = "ALT-CTRL-END", macrotext = "/cast 割碎"},--111
			{framename = "DA_CAST_REGROWTH_P1", button = "ALT-CTRL-PAGEUP", macrotext = "/cast [@party1,exists]愈合"},--112
			{framename = "DA_CAST_REGROWTH_P2", button = "ALT-CTRL-PAGEDOWN", macrotext = "/cast [@party2,exists]愈合"},--113
			{framename = "DA_CAST_REGROWTH_P3", button = "ALT-CTRL-UP", macrotext = "/cast [@party3,exists]愈合"},--114
			{framename = "DA_CAST_REGROWTH_P4", button = "ALT-CTRL-LEFT", macrotext = "/cast [@party4,exists]愈合"},--115
...
FeralSetBinding:SetScript("OnEvent", FeralSetMacroButton)
FeralSetBinding:SetScript("OnUpdate", FeralSetBinding_OnEvent)