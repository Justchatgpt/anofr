...
local NoClearDebuffCache = {
	...
	["培植毒药"] = {SpellID = 461487, Instance = "艾拉-卡拉，回响之城"},
}
...
function Feral_ScanUnitAuras(unitid)
	...
	if DA_IsSpellInRange(Regrowth_SpellID, unitid) == 1 and not Feral_GetTargetNotVisible(unitid) and DA_GetLineOfSight("player", unitid) and not UnitIsCharmed(unitid) and UnitReaction("player", unitid) > 4 and UnitIsConnected(unitid) and UnitCanAssist("player", unitid) and UnitIsVisible(unitid) and UnitPhaseReason(unitid)~=0 and UnitPhaseReason(unitid)~=1 and (UnitInRange(unitid) or not IsInGroup()) then
		local index = 1
		while true do
			...
			if debuffType == "Curse" or debuffType == "Poison" then 
				if MouseoverUnitTime < 0.3 or not FeralSaves.FeralOption_Auras_ClearMouseover then
				...
				if debuffType == "Curse" then
					if (FeralSaves.FeralOption_Auras_ClearCurse or (FeralSaves.FeralOption_Auras_ClearMouseover and MouseoverUnitTime >= 0.3)) then
					--自动解诅咒
						if not RemoveCorruptionCD and not IsStealthed() and not Feral_CastSpellIng and (not Feral_AutoDPS_DPSTarget or UnitPower("player", 3) < 70 or (DamagerEngine_GroupMember and #DamagerEngine_GroupMember <= 1)) then
							...
							Feral_SetDebugInfo("清除腐蚀")
						end
					end
				end
				if debuffType == "Poison" then
					if (FeralSaves.FeralOption_Auras_ClearPoison or (FeralSaves.FeralOption_Auras_ClearMouseover and MouseoverUnitTime >= 0.3)) then
					--自动解毒药
						if not RemoveCorruptionCD and not IsStealthed() and not Feral_CastSpellIng and (not Feral_AutoDPS_DPSTarget or UnitPower("player", 3) < 70 or (DamagerEngine_GroupMember and #DamagerEngine_GroupMember <= 1)) then
							if GetCVar("autoSelfCast") == '1' and UnitIsUnit('player', unitid) and (UnitCanAttack("player", "target") or UnitIsEnemy("player", "target")) then
							--施法目标是玩家自己且玩家自己的目标是可攻击或敌对目标
...
function Feral_GetGroupWithUnitCount(unitid, distance)
	--获取目标附近一定距离内队友数量
	local PartyCount = 0
	if IsInRaid() then
